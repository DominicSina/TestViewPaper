...
This brings us the problem that while testing clearly has its benefits it is still seen as optional or too time consuming. That many developers underestimate the difference that unit tests can make becomes evident in that even when there are tests they are not executed as often as they should be. In their case studies Pressman and Ernst found that the 31-82\%  reduction of waisted time can be achieved by simply running the same tests two to five times as often\cite[p.~1]{Pres14a}. The reason why the effect of testing is underestimated is that often under stress the more direct and optimistic way of implementing more features instead of testing seems faster. This will in the long run slow down the development process since all the previously discussed benefits of testing are missing. So there might be no time for testing in the future as well. 
...
This means that the scope of each test is very small and consequently many tests are needed to cover the whole implementation. By making unit tests one ensures that the tested method behave as expected.
...
\end{quote}
While it might seem tedious this repeated careful planning is the biggest advantage of this approach. Applications developed with Test-driven Development have to be very thought-out and every requirement has to be clear since those are required to begin writing the the production code.
...
As shown it does not matter if the test class and package already exist or if they need to be added. Creating new tests can be facilitated either by letting the user quickly specify existing test classes where the test will be added or by making creating new test classes easier. Providing a default name for new tests can help in both cases. Another help would be the ability to let the user create these packages, classes and tests together instead of individually.

If the default names are missing the developer has to enter similarly structured names for packages, classes and test over and over. In case test package, classes and methods can not be created together the user will have to click around in the environment multiple times to start the creation of each of these. While it helps to have these aids a lack of them seems not as bad as a lack of the test search functionality described previously in \autoref{sec:Showing all tests to a method}.
