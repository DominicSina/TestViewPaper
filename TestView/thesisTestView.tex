...
extends ``\emph{TestCase}'' and that the test name starts with ``test''. The ``@Test''
...
These names and the inheritance from ``\emph{TestCase}'' is also what Nautilus's test
...
One image had the TVPlugin installed and the other did not. In each image one
...
and various windows opened inside of Pharo \cite{Mine2016a}. We modified DFlow slightly to correctly keep track of mouseclicks and made it log the gathered data locally instead of sending it to a server.
...
\begin{table}[h]
	...
	\hline
   	\end{tabular}
	\caption {Results with the TVPlugin}
	\label {tab:eval1}
\end{table}
...
\begin{table}[h]
	...
	\hline
   	\end{tabular}
	\caption {Results without the TVPlugin}
	\label {tab:eval2}
\end{table}

The averages of the experiments with the TVPlugin seem to be slightly worse
...
tests the user wants to see in this second editor from what is displayed in the
first code editor.
...
test class will not be found and thus not be shown in the found tests drop down
...
verify if it now works by opening the found tests drop down list.
...
First make sure that your test classes are subclasses of ``\emph{TestCase}''. If they
...
``\emph{[YourClassName] subclass: \#TestCase}''. Check again if the tests from your
