...
\section{What's the TestView Plugin?}
	\label{sec:What's a TestView Plugin?}
...
With the ``Unlink Class'' button you can exclude classes from being searched for tests. Like the ``Link Class'' button this exclusion will only count for the class of the method that you have currently selected in the Nautilus class hierarchy. Detailed instructions on how to use these functionalities are found in \autoref{sec:Linking an existing test class} and \autoref{sec:Unlinking an existing test class}.
...
\section{Adding a new test inside a new test class}
	\label{sec:Creating a new test inside a new test class}
So now that the plugin is set up after following the steps in \autoref{sec:Installation and activation} let us add a new test to a method. It is assumed here that you already have a method to test named ``doSomething'' inside of a class named ``MyClass'' and a package called ``MyPackage''.
...
\section{Adding a new to an existing test class}
	\label{sec:Adding a new to an existing test class}
	In this section we will take a look at how to add a new test to an existing class. It is assumed that the plugin is activated. To do this you have to follow the steps outlined in \autoref{sec:Installation and activation}.
...
\section{Linking an existing test class}
	\label{sec:Linking an existing test class}
...
\section{Unlinking an existing test class}
	\label{sec:Unlinking an existing test class}
...
\section{Troubleshooting}
	...
	\subsection{Tests are not found by the TVPlugin}
		\label{subsec:Tests are not found by the TVPlugin}
...
If this does not help you can force the plugin to recognize your classes as test classes by linking the missing test classes to the currently selected class in the Nautilus class hierarchy. To do this follow the steps in \autoref{sec:Linking an existing test class}. Recheck if your tests are found. If this does not work either then your tests themselves are written in a way that the TVPlugin does not recognize. You will have to change them so that they fulfill certain criteria. In this case also continue with \autoref{subsec:Add and remove individual tests from the corresponding test list to the selected method}.

\subsection{Tests that do not test the currently selected method are shown}
		\label{subsec:Tests that do not test the currently selected method are shown}
If the classes that contains these wrongly indentified tests do not contain any tests for the currently selected class then you can simply unlink these classes. Follow the steps outlined in \autoref{sec:Unlinking an existing test class}. All the tests from these classes should new be gone from the found tests droplist.

If you want to remove only some tests from the found tests droplist then continue with \autoref{subsec:Add and remove individual tests from the corresponding test list to the selected method}. 

\subsection{Adding and removing individual tests to the tests found by the TVPlugin}
		\label{subsec:Add and remove individual tests from the corresponding test list to the selected method}
...
\meta{naming criterium explained}
The name based criterium is a check if your test contains the full name of your method under test with at least one additonal time the substring "test". Examples of what is recognized as a test and what not based on this criterium are shown in \autoref{tab:name criterium examples}. This check is not case sensitive. And ":" from methods names with parameters will be ignored when looking for a test.

\begin{table}
   \begin{tabular}{| l || c | r |}
     \hline
     Original method name & Test names & Not test names \\ \hline
     doStuff & doStuffTest, testDoStuff, TestdosTuff & doSomething, doTestSomething, testSomething \\ \hline
     do:on: & testDoOn, doOnTest & testDo:On:, do:testOn:  \\
     \hline
   \end{tabular}
	\caption{Test naming criterium}
\label{tab:name criterium examples}
\end{table}

\meta{cotains method selecor the of the original method criterium}
The second criterium is if a method call is done inside of the possible test method to a method with the same name as the in the Nautils class hierachy selected method. This time the ":" are not removed from the check. If the supposed test method to a method named "do:on:" does call "doOn" then it will still not count as a test.

\meta{fulfill any criterium to add test to the testlist, break both to remove it from the list}
