...
chose to realize this as a Nautilus plugin since this makes it easy to set up
and builds on established parts of the Pharo environment. This plugin includes
...
for improvement. The aim of this thesis is to take a closer look at unit testing and in form
...
stress}\brs{replacing pressure with stress does not solve the problem. What
does the ``especially under stress'' mean?} unit testing is still seen as
...
their case studies Pressman and Ernst found that a 31-82\% reduction of waisted
...
In unit testing it is important that each method is tested (baring trivial
...
the full name of the selected class in addition to ``test''. Unlike Nautilus' naming criterion this substring
search is not case sensitive and it does not matter which of those substrings comes first or if the name
...
More precisely the substring search works as follows. First all class names
\ins{from bla bla }are gathered in a \textbf{class name collection}. \chg{In a
\textbf{substring collection} the name of the class under test and the string
``test'' is put}{The name of the class under test and the string ``test'' are
put in the substring collection}.  The substring collection is sorted so that
the longest string is first.

\ugh{Starting with the longest substring out of the substring collection each
element in the class name collection is now checked if it contains this
substring.  If it does the first occurrence of this substring is removed and it
is checked if the remaining string contains the second substring from the
substring collection.  A class name qualifies as test class name if both
substrings from the substring collection were found in it.}\brs{so, essentially,
you just check which potential test classes contain the name of the class in
question and contain the string ``test'' outside the class name. Please say
this simply, no need to name collections and complicate}

To illustrate this substring search let us examine why a class named
``Contest'' is not considered a test class for a class named ``Contest''. To
confirm to the naming criterion used by the TVPlugin the test class name would
have to contain the substrings ``Contest'' and ``test''. The substring search
first checks if the substring ``Contest'' is included in ``Contest''. Since it
is the first occurrence of  ``Contest'' is removed, which leaves an empty
string. Now it is checked if this empty string contains ``test'' as well.
Since it does not ``Contest'' is not a test class for ``Contest''.
...
corresponding test classes. Then the substring collection is filled with the
name of the selected method and ``test''. The only difference here is that all
colons are removed from the name of the selected method before it is put in the
substring collection. Like before the substring collection is sorted according
to length, both substring checks are made and matches removed. If both substring
checks are successful the method is a test for the selected
method\chg{-}{.}\brs{same as earlier, keep it simple, no need to name the
collections, just say what you did}
...
the topmost element ``new Test'' from the drop down list of found tests as show in
\autoref{fig:newTest}.
...
enhancing the existing tools would to keep the number of required tools low.
...
missing tests in the found tests drop down list.
