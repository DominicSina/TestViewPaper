...
This means that the scope of each test is very small and consequently many tests are needed to cover \chg{a}{the} whole implementation. By making unit tests one ensures that the tested method behave as expected.\del{and also speeds up development since these automated test are very fast to execute.} \brs{writing tests speeds up development(not true) because tests are fast to execute(makes no sense)} 
...
\dos{did breaking it up into sections help?}
...
%%%%%%%%%%%%
\todo{continue reading through this}
%%%%%%%%%%%
...
%Both of those derivations should be set as default values for the new class and package to possibly save some time. In Java these name derivations are often quite easy to make\cite[p.~19]{Mars05a}. It can be assumed that in many other languages this will be the same.
