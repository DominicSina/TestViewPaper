...
This brings us \ins{to} the problem that, while testing clearly has its benefits, it is still seen as optional or too time consuming.
...
\brs{add a paragraph about the problem and one about what has been done in the thesis}
...
Unit testing follows the paradigm of isolating the smallest inseparable parts of a program and testing them independently of each other. Each of these tests is done \chg{as low level}{at as low a level} as possible. System wide tests are generally not a part of unit testing.
This means that the scope of each test is very small and consequently many tests are needed to cover the whole implementation. By making unit tests one ensures that the tested parts behave as expected. In Smalltalk these smallest units are often methods since the language heavily relies on sending messages \ugh{(what roughly translates to a method invocation in other languages)}\brs{make this a footnote}  between objects.
...
An example for preconditions would be the requirement for the objects instance variables to have certain values. From this initial state in a deterministic environment a specific outcome can be expected after the method is executed. The outcome is a success if all of the previously defined postconditions are fulfilled.\brs{You might want to mention assertions here}

To illustrate this let us examine how unit tests for a \ugh{simple stack}\brs{You are using the actual Pharo stack implementation in the code later, so you might as well say so} might work. The stack has a maximum capacity that is set once the stack is created. Further the stack provides a method named \chg{"}{``}\chg{push(x)}{\emph{push}}\chg{"}{''}\brs{fix quotation marks everywhere else, use emph for code chunks in text, and } which adds an element "x" on top of the stack and  a method named "pop()" which removes and returns the element that was added last to the stack. Now let us examine how to make a unit test for this stack that checks if the last element that was added is returned by \chg{"pop()"}{the pop operation} and not some other object.
...
Now we need to make sure that operations work as expected\ins{.} \chg{or in}{In} other word\ins{s} we need to check \chg{our}{the} postconditions. In this case if the object returned by "pop()" is the one that we pushed to the stack and that the \ugh{stack is empty again after the element was popped}\brs{you did not mention this a paragraph ago}. Below is an example in SmallTalk of how such a test could look like. To check if the stack is empty again after calling pop() a separate test could be written.

\brs{the last two paragraphs are very ugly. Make this code a listing of it's on and reference it early in the text, then explain it. You are doing the opposite, you first explain a lot of things, forcing the reader to keep it in their mind, and then you show it to them. First show, then explain, that way you can reference the listing and the line of code you are talking about. Also, make sure the listing has line nubmers}
...
For the purpose of this \ugh{paper}\brs{paper?} two features of unit testing are important to keep in mind. The first one is that unit testing takes place at a method level and the second one is that the narrow scope of unit tests requires many tests to cover multiple methods. Further, \chg{due to how}{since} it is recommended to keep each test as small as possible even a single method is likely to require multiple corresponding tests since every path in the decision tree of the method should be covered.
...
After this more tests are added to test additional features. This process is repeated until the implementation is sufficient\del{ly working}.
Test-driven development thus requires the developers to regularly determine the requirements for a feature\brs{and normal development does not?}.
...
As shown in \autoref{sec:Unit Testing} \ugh{it is fair to look at testing on a method level when discussing unit testing}\brs{does not parse. No idea what this is suppose to say}. \ugh{Since in Test-driven Development the methods may not have been written when the tests are created the method declarations and the method locations in the class hierarchy can not always be used to make certain assumptions that could be used to facilitate creating new tests}\brs{sentence is way too long}. For example \ugh{before a method in the production code is written it needs to have a failing tests}\brs{how is this an example?}.  \ugh{Only for subsequent tests \chg{to}{of} the same method can the method declaration and the method location in the class hierarchy be found in the production code.}\brs{hm...  ok... ``we can only find the method declaration and location for methods that already have existing tests'' or something.}

\brs{you spend a lot of time in these paragraphs explaining how we don't know
things about the methods because we don't have them yet, but I have no idea why
that is important to know. It should be clear WHY not knowing this is a
problem}

Test-driven Development bears mentioning because it is one of the major paradigms concerned with testing and because in the context of this \ugh{paper}\brs{s/paper/thesis/g or something} it has to be treated a bit differently. Namely since it is not possible to always get the method declaration and location in the class hierarchy to help generating tests when doing Test-driven Development. \brs{AHA! Now you mention, yet just breafly something about generation of tests! Just a glimpse, but not enough.  That is why you need to make clear in the introduction what exactly you are trying to do}

\section{Blackbox and Whitebox Testing}
	\label{sec:Blackbox and Whitebox Testing}

Whitebox and blackbox testing refer to how closely tests are written to suit a method. In whitebox testing the tester is completely aware of the method and its inner workings when writing a test for it. This means that an adequate amount of unit tests done with a whitebox approach will cover each important path in the decision tree of a method. \ugh{Especially interesting are worst case scenarios that can be found by examining the code and method invocations that result in as much executed lines as possible}\brs{convoluted. Are method invocations interesting?  cause if not, you are missing a ``the'' in front of method invocation. Do the method invocations result in as much executed lines as possible? Please rephrase the entire sentence}.

Contrary in blackbox testing the test writer does not know the inner workings of the method that is being tested. Ghezzi et al. describe that during blackbox testing "[t]est\brs{lose the square brackets} sets are developed and their results evaluated solely on the basis of the specifications" \cite{Ghez02a}.  This means that the tests have to be made so that they return the right value for all tested input values. Since it might be \chg{unfeasible}{impossible} to test all values\brs{values of what?} it is necessary to choose the critical input values where an error might occur.
...
As shown in \autoref{cha:introduction} some current software development paradigms include unit testing\ins{.} \chg{but}{Still,} as important as unit testing is\ins{,} not all current development environments are optimized for it.  For example, the search for unit tests \chg{to}{of} a specific method is often lacking or not present at all.

In this work we assume that important features for unit test friendly environments are\ugh{ a quick way to see if a method is tested, an automated test search, the ability to view tests and methods side by side and an option to easily create new tests.}\brs{enumerate this since it matches following subsections} These criteria are based on the definitions of the various testing paradigms previously described in \autoref{cha:Testing}\brs{i don't see how these are based on that. And I stated so in the previous revision: ``what? They are based on testing paradigms?  How?  In what way?'' commit b630a67c884efdc35f903de7a715b1c6450683e1. The point is that you need a better bridge between this paragraph and the next. Starting a new paragraph means that this thought is over, and it is clearly not}.
...
Since not many environments were created with specifically unit testing in mind \chg{there are often}{,} some of these features \ins{are often } missing.  Concrete examples of environments lacking these specific features will be pointed out in \autoref{cha:Related Work}. To compensate for these missing features the user is required to manually execute many tasks like navigating back and forth between method and test and creating new test classes and packages. This repeatedly breaks the programmers flow and in effect discourages them from writing tests.  In the following \brs{the following what?} we will take a closer look at the \ugh{tasks involved in different goals arising from unit testing which will help to understand the problem of those missing functionalities and exactly at which point they are needed}\brs{convoluted.  Please top writing sentences like ``we will look at thing that does the stuff while breaking things that correlate to stuff that is used in things}.

\section{Is a method tested?}
\label{sec:Is a method tested?}

In unit testing it is important that each method is tested\ins{(baring trivial things like getters and setters)}. As discussed in \autoref{sec:Unit Testing} a method with no tests is unsafe and might even have to be tested manually which is quite slow. Thus a very important feature in unit testing is the ability to check if a method has at least one test. This has to be as easy to see and as \chg{uninterruptive}{non intrusive} as possible.

If this feature is not implemented users have to switch back and forth between a class and all its test classes to check manually. If the test classes are not known to them they have to find them first. Both of those tasks can take a lot of effort.  Luckily code coverage tools \ins{like CodeCoverageToolA or CodeCoverageToolB}  also provide this functionality. They allow the user to run test suits and then see which part of the implementation were executed. This will show if a method never got called and thus is untested.\brs{problem with this is that the method could be covered indirectly, by being called by a tested method, without being tested itself.}
...
Similar to showing if a method is tested this helps the user to decide if it is necessary to write a new test. Further this functionality helps the user to see if a certain test case has already been created and thus is not need\ins{ed} again. A \chg{user}{developer} could also study how a method works by looking at the corresponding tests.

A lack of this test search function will require great effort on part of the user to keep track of  test classes and test methods. If the user decides to add a new test then it is necessary to take a short look at every test to determine if a similar test has not been written. In this case the tester needs to have quick access to all tests of this method. A lack of this feature \chg{makes}{might make} \brs{you have no proof of this} unit testing in \chg{anything but the smallest applications and test suits}{larger applications and test suits} very tedious and new \chg{testers}{test writers} will have trouble getting an overview of the existing test suit.

Code coverage tools can not fully substitute an automated test search since it relies strongly on the relation between tests and method.  To be useful the code coverage tool would have to save for each method from which test it was called during the execution of a test suit. Also coverage is not \chg{necessary}{necessarily} a good metric to determine if a tests actually tests a method. Nested method calls might yield a large number of false positives.\brs{yes, this is what I meant in the previous subsection. Mention it there as well}

\section{Creating new tests}
\label{sec:Creating new tests}

As discussed previously it should be as simple as possible to check if a method is not sufficiently tested. \ins{If this is the case} \del{Almost as a direct consequence} the user has to create these new tests \del{if this is the case}.  Especially at the start of a project many new tests will have to be created. So it is safe to say that \ins{another reoccurring user goal}in unit testing \del{another reoccurring user goal} is the creation of new tests for \ins{insufficiently tested} methods \del{with no or too few tests}.

While creating a new test the \chg{user}{developer} \brs{be mindful who is a use and who is a developer. You are not talking about your tool here so there is no user} first has to decide \del{in} which package and class the new test will be added \ins{to} and how the test will be called. In many cases the location of this new test has already been decided on since other tests corresponding to the same method are already there. In this case a quick option to specify an existing test class should be offered.
...
If the default names are missing the developer has to enter similarly structured names for packages, classes and test over and over. In case that test packages, classes and methods can not be created together the user will have to \ugh{click around in the environment multiple times to}\brs{what if there are keyboard shortcuts? rephrase}  start the creation of each of these.  While it helps to have these aids a lack of them seems not as bad as a lack of the test search functionality described previously in \autoref{sec:Showing all tests to a method}.

\section{Methods and Tests Side by Side}
\label{sec:Methods and Tests Side by Side}

In whitebox testing it is required to know a method's inner workings. It is expected that the programmer who writes the test is trying to test every line of code. Thus it is important to provide the user with information on how the method works. \del{The} possibly \ins{the} simplest way is to let the users see the whole \ins{source code of the} method in question at the same time as they are writing a test for it.
...
between gathering information about a method\chg{,}{and} writing tests for it
\del{and looking up the same information again} can save valuable time and effort.
...
In this section \ins{we describe} current programming environments, namely Eclipse and Pharo, \del{are analyzed} \chg{in}{with respect to} how unit test friendly they are. Special focus will be placed on the features described in \autoref{cha:The Problem}. These features are: viewing test and method side by side, search all tests to a specified method and facilitating the creation of new tests. The two environments \chg{will be}{are} compared and possible places for improvement \chg{will be}{are} discussed.

\section{Unit Testing in Eclipse}
\label{seg:Unit Testing in Eclipse}

We will start by taking a closer look at Eclipse since \chg{its use is very widespread}{it is widely used} and \del{it has} many parallels to other programming environments \chg{like for example}{(\eg} Visual Studio \ins{) can be made}. In \autoref{fig:Eclipse's user interface} you see a Java project opened in Eclipse with a package and two classes. \ins{the package and classes can be seen on the left side in the ``Package Explorer'' pane.} \brs{You might want to consider increasing the font on these images, this is way too small to be read}
...
\chg{Let us}{We} start with \chg{the}{a} discussion \chg{of}{on} how \del{ in} Eclipse \ins{enables} \del{the} users \chg{can}{to} see the \ugh{tests that they are currently writing and the methods that are being tested at the same time}\brs{too flowery, try something like: method and corresponding test at the same time}. In \autoref{fig:Eclipse's user interface} on the left hand side of the window you can see the Package Explorer. \ugh{In this file hierarchy you can see every project, package and class in the current workspace.}\brs{I don't see how this last sentence is related to the ones before. Then, you continue talking about similar things in the next paragraph. Seems to me you have a tendency for bad choices on where to break paragraphs. A paragraph should be one continuous thought.}

Classes that are opened through the Package Explorer will be shown in the middle of the screen and a new tab on top of the code editor will appear.  In \autoref{fig:Eclipse's user interface} there are two tabs "ClassA" and "ClassB".  The "ClassB" tab is currently selected and thus the content of "ClassB" is displayed in the code editor.  Unless you close these tabs all the classes you opened will be quickly available through their tabs.\brs{I really don't like foo/bar examples. Please try to open a real project with real classes}

A big advantage of this system is that users can create favorites by not closing important tabs. Through this they can switch back and forth between methods and tests. A slight drawback is that the user has to close unneeded tabs from time to time\brs{Why does he HAVE to close them?}. In most cases the use of these tabs will be smooth enough to allow the programmer to switch uninterrupted between tests and methods \ugh{via their classes}\brs{not sure what you mean by this}.

If for some reason this does not suffice then it is also possible to split the code editor multiple times either horizontally or vertically like in \autoref{fig:eclipseSplitscreen}. Since each code panel is smaller than the original one this splitting can only be done a \chg{few}{a limited number of} times. After this the panels get too small to be useful.

As shown \ins{,} Eclipse provides multiple ways to make the tested method easily accessible during testing.

\subsection{Test Search} Another feature we discussed in the previous chapter
was the ability to find all tests \chg{to}{of} \brs{apply this globally. a test
is OF or FOR the method not TO the method} a certain method. Eclipse provides a good option to search for tests. By \chg{pressing}{selecting the} "Se\underline{a}rch"\brs{no need for the underline} from the top menu bar followed by "Referring Tests..." it is possible to find all tests that reference the currently selected method.

Sadly this feature is very badly documented. The requirements for a test to \chg{qualify as a test to a}{to be associated with} certain method seem to include that the test class extends "TestCase" and that the test name starts with "test". "@Test" annotations are ignored by this function. It seems like this test search does not yet support JUnit 4 standards.
...
The next feature in Eclipse we now discuss is how the creation of new tests is facilitated. Eclipse \chg{has}{provides} the option \chg{to}{for} add\ins{ing} a new test class by right clicking on the class that should be tested, selecting "New" from the list that is shown and then clicking "JUnit Test Case".  \ugh{There are other ways to do this but by making sure that the right class is selected in the Package Explorer Eclipse is able to support the user better}\brs{does not parse. No idea what you are saying here}.
...
On one hand\ins{,} this wizard is very good for creating new test classes but on the other hand Eclipse helps neither to add new test methods in an already existing test class nor to create more than one test per method.
\brs{also, not too helpful for TDD}

\section{Unit Testing in Pharo}

Now let us take a look how the Pharo IDE and its system browser Nautilus \ugh{compare to Eclipse}\brs{why? Are we not suppose to be looking at how it supports the three problems discussed not compare to eclipse}. It is worth mentioning that the Pharo IDE is quite different from Eclipse in that the placement of its visual elements is less rigid\brs{hm... Really? I would not say that is necessarily true.}. The users are able to customize the appearance of the environment very quickly and adapt it to their wishes. On the other hand Pharo is less wide spread and not as well maintained in many ways as Eclipse.
...
As with Eclipse the first functionality we will look at is the ability to view tests and methods side by side. The Pharo IDE provides a very customizable environment and it is \ugh{almost necessary for a user to arrange all windows that are created in a way that is comfortable}\brs{rephrase}. It is possible to create multiple Nautilus windows and arrange them so that \del{in} one \ins{shows }the method and \del{in} the other the test for this method \del{is shown}. Through this a very similar effect to Eclipse's split code editor is achieved.
...
A possibility that does not have this drawback is to lock a method or class and then select others inside the same Nautilus window. It is possible to select which of the locked elements is shown in the code panel by using the "All", "Current" and number buttons shown at the bottom of \autoref{fig:Locking}\brs{larger font in the picture pls}.  Through these buttons the users can switch between seeing all locked methods and classes at the same time or each individually.  This is very similar to the tabs of opened classes that Eclipse has. Although with many locked classes and methods it is hard to remember which number belongs to what.
...
The next functionality that will be discussed is how Pharo finds existing tests \ugh{to} a specified method. Like Eclipse Nautilus has \ugh{some sort of}\brs{``some sort of'' is not really a phrase one wants to see in an academic text} test search implemented. Like using Eclipse's test search only tests with a very specific name are found. Additionally they have to be placed in classes with an equally specific name. Examples of tests corresponding to certain methods can be found in \autoref{tab:correspondingMethods}\brs{first explain, then give the example. Having the reference to the table here serves no purpose and the reader will look at the table and try to figure out what is the rule, and that is your job to explain. You seem to do that in the next paragraph, but by that time, the reader has either given up, or figured it out and will be annoyed that you explain it when he wasted time.}.
...
Namely\ins{,} test classes and test method\ins{s} have to contain the full name of the \ugh{original}\brs{which is the original? does that mean that the other ones are unoriginal?} class or \del{respectively} method. Additionally the name of the test class has to have the suffix "Test" and the name of the test method has to have the prefix "test". Also tests have no parameters and thus lack all colons in their name.  If anything more is part of the test name then it will not be found\ins{ by the ...}. The test classes also have to inherit from TestCase in order to be found by the test search.

Both the restrictions on class names and method names are quite restrictive.  From a test class name the class under test can be quite reliably inferred\cite{Mars05a}. On the other hand Marschall \ins{who?} also states that the method name and the corresponding test name for it are less directly related. These restrictive name criteria also lead to the problem that at most only one test per method will be found since others with a slightly different name will not fulfill these naming criteria.
...
\subsection{Creating new tests} \del{In Nautilus with a} right click\ins{ing} on a method ins{in Nautilus gives } the \del{option} "Generate test" \ins{option}\del{is given}.  The new test will automatically be added to a certain test class in a specific package (both of which will be created if needed). More precisely the test package is named like the \ugh{original}\brs{original is not the word you are looking for. Replace globally} package with the suffix "-Tests". The test class and method will be named like \del{shown in} \autoref{tab:correspondingMethods}\ins{shows}.  All the tests created through this will thus be found by Nautilus's test search function.  "Generate test and jump" works very similar but also selects and shows the newly created test so that the user can start implementing.

Using these predefined package and class names is faster since the user does not need to enter them. Another advantage is that tests created in this manner confirm to the naming standards imposed by Nautilus. Sadly this robs the user of the ability to specify different package and class names. Also only one test per method can be created in this manner. All additional tests for the same method will simply overwrite the previous test\brs{are you sure about this? If I have a test and click the generate test button it will erase the old test?}.
...
Having discussed some important features of a unit test friendly environment in \autoref{cha:The Problem} and where their implementation is lacking in \autoref{cha:Related Work} we will now \ugh{try to present}\brs{we will not TRY to present, we will present} \ugh{a solution}\brs{a bit of a bold statement that testView is a solution to these problems. I would prefer something like ``our attempt to address the problems''} to this problem: the TestView Plugin.

Since the lack of unit test supporting features can not be corrected for all environments at once we chose to extend Nautilus's functionality. The reasoning behind this was that the inability to view tests and methods side by side in a single Nautilus window was perceived by us as the gravest of these issues and that Eclipse already has a big community which provides many plug-ins.\brs{the reasoning is weak. This is not why we chose to do it for Nautilus. It would be better to say that we chose it because it is easier to implement then an eclipse plugin}

\ugh{In this section we will comment on the same three functionalities discussed in the last chapters: viewing test and method side by side, finding tests to certain methods and creating new tests. Special focus will be put on improvements compared to Eclipse and the Pharo IDE.} \brs{What does this mean?  We will talk about something we already talked about! rephrase to put an accent on testView}

\section{Methods and Tests Side by Side}
As stated\ins{before/in section x,} one of the drawbacks concerning unit testing with Nautilus is that it is hard to view tests and methods at the same time. Whitebox testing becomes cumbersome through this. Seeing both method and test together should be quick\chg{,}{and } convenient\ins{.} \chg{and not}{It should also not} introduce too much redundant user interface elements that might clutter \ins{up}  the environment.

Following this\brs{You do this all over the paper, if you want to start a sentence with a reference to somethin earlier, like: ``Following this'' or ``With this in mind'' you need to put a comma after. This separates the two parts of the sentence clearly. Please apply this globaly} the TestView Plugin allows \chg{to split}{splitting} the code editor panel of a Nautilus window vertically into two parts\chg{,}{.} \chg{the}{The} result \ins{of this} can be seen in \autoref{fig:Nautilus window with two code editors}. With this approach the need to open two Nautilus windows is eliminated. The file hierarchy for example will not be displayed a second time and less screen space is occupied.
...
In \autoref{tab:TVTestClassNames} examples of which corresponding test
classes that the TestView Plugin finds are shown.
All classes with names like in the column of possible class names are considered
corresponding test classes if they also inherit from TestCase.
\brs{way to complicated, and poorly explains what you are
trying to say.  Make clear that you find test classes by looking at subclasses
of TestCase.  also, explain and then give examples, like mentioned before. If
you give examples first, then use the example to explain.}
...
The name-based criterion requires the name of the class in question to contain both "test" and the name of the selected class.  The substring search is not case sensitive and \ugh{the matches for "test" and for the name of the selected class can not overlap} \brs{wat?}. This is why "Contest" is not considered to be a test class to a class that is also named "Contest" even though it contains "test" as well as the full name of the class under test.

\ugh{Since the results of this search for corresponding classes \chg{are}{is} used as \ins{a} base for the search for corresponding tests it is important that it works as well as possible} \brs{Very ugly and says nothing. This is used for that so it is important that it works well}. If this search for corresponding test classes is inadequate the developers can use the "Link Class" and "Unlink Class" buttons provided by the TestView Plugin to add and remove test classes from the results.  \brs{This is not a good explanation of why one needs these buttons, and what they do. Rewrite.}

\subsection{Corresponding Test Methods}
After some possible test classes have been identified using the criteria described in \autoref{subsec:CorrTestClasses} the result\chg{s}{ing classes} will be searched for corresponding test methods to the currently selected method.

Similar as the name criterion for test classes described in \autoref{subsec:CorrTestClasses} the test\chg{s}{methods} have to contain the full name of the selected method\ins{ }(without colons) as well as \ins{the string} "test". Examples can be found in \autoref{tab:testNameCrit}.
...
\brs{Please use real-world-ish examples. Foo bar/do stuff test should be
avoided}
...
\brs{so, you are flying over most of the things you did in your project. You don't talk about your implementation at all \ie how you do the doStuff--testDoStuff mapping (regex or what?), The way to implemented the sorting of potential test methods is just mentioned in one sentence. I feel you are missing a lot}

\section{Creating new tests}
The purpose of \ugh{this}\brs{which?} functionality is to \ugh{prevent the users from having to}\brs{ugly phrasing. you don't prevent someone from having to do something you enable someone to do something better} repetitively enter similarly structured class and package names while still letting them retain the freedom to choose each name if needed.

Every time developers \brs{please use singular} start to write a new unit test they have to determine to which test class the new test belongs. We can split the creation of new tests into two basic use cases: adding the new test in a new test class or adding the new test in a test class where \del{there already are} tests corresponding to the selected method already exist.

\subsection{Adding a new Test Class}
First let us talk about what can be done to facilitate the creation of a new test that does not yet have a fitting test class. The easiest way to do this is to let the users write the new test and later determine where this test will be \chg{put}{placed}. With this the user is encouraged to immediately start writing a test as soon as a method is created.

When as the test \ins{method}\brs{there is a difference between a test and a
test method, try to use the right phrasing.Apply globally}  is saved the plugin will ask the user \chg{how the new test class should be named}{for the name of the test class} and in which package this class should be\chg{put}{placed}. \del{Here} default values\ins{,} based on the class name and the package name of the method under test\ins{,} are \chg{given}{provided by default.} \chg{which in many cases}{This} might already be sufficient since test class names and test package names can often be derived from the original class and package names \cite{Mars05a}.\brs{please explain how you come up with the default name, example is not enough}
