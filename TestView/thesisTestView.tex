...
by side with limited but more focused content in the right code editor.
\ugh{Being able to view method and test side by side results in an easy to use
help for whitebox testing.}\brs{this does not address Oscar's comment. HOW does
it help? By providing the developer with the method implementation to inform
decisions while writing tests or something. Also, ``easy to use help'' is not a
good formulation} Since it is not possible to view multiple methods in one
...
their test class is not considered to be a test class \chg{to}{for} the
currently \del{in Nautilus} selected method. All test methods in this missing
test class will not be found and thus not be shown in the found tests drop
list. Here you find step by step instructions \chg{of}{on} how to add
\chg{your}{a missing} test class to the considered test classes.

\begin{enumerate}

\item Make sure that\ins{,} in the Nautilus class hierarchy\ins{,} you have
