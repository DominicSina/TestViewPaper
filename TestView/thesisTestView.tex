...
\section{The Setup}
%the participants
We evaluated the plugin with four \chg{people }{PhD students} from the Software
...
\del{previously} selected two problems \ins{in advance}. The participants were
asked to write in the Pharo IDE a solution to these problems. One problem was
the ``Even Fibonacci
numbers''\del{-}problem\footnote{https://projecteuler.net/problem=2} and the other
problem was the ``Sum Square
Difference''\del{-}problem\footnote{https://projecteuler.net/problem=6}.
...
Implement the ``Even Fibonacci numbers''\del{-}Problem.
...
collection\brs{didn't this and the next method have  names as well?}.
...
Implement the  ``Sum Square Difference''\del{-}Problem.
...
of the tasks had to be implemented. DFlow\brs{link in the footnote, or even
better cite the paper}\ins{, a tool for bla bla bla,}\brs{not everyone knows
what dflow is} was used in both images to track the user interactions and the
time.

\chg{To prevent}{In order for} the order of the images and the difficulty of
the problems \chg{from influencing}{not to influence} the evaluation we used
counter balancing on both of these factors.  This means that the first two
people started both with the ``Even Fibonacci numbers''\del{-}Problem while the
...
\ref{itm:q2} two participants \chg{each said to}{stated that they} write unit
tests rarely\chg{or often respectively.}{and two stated that they write unit tests often}
...
than those without. This is very surprising since except \chg{person
two}{participant number two}\brs{apply globally} nobody used or noticed the
plugin. Most likely the small number of participants made the differences
between them the decisive factor.

Most people with the exception of person two did not do unit testing or make
use of the TestView plugin. The one person that used the plugin has a test
coverage of 100\% with or without the plugin. Also Person two was faster,
pressed less \chg{keyboard}{keys} or mouse buttons and opened less windows
without the plugin. This difference is most likely due to the varying difficult
of the problems. Everybody seemed to have more difficulty with the
\hyperref[prob:fib]{Fibonacci problem}.

\ins{Is is }Interesting \chg{was}{to note } that while only one person wrote
...
could possibly be annoyed by it standing out to much.

A quick option to run all the found tests would certainly be a useful addition
to the plugin. Luckily there \chg{are}{is} already the Nautilus
TestRunner\ins{a tool that bla bla bla} \chg{and}{.} Nautilus itself
\chg{that}{also} provide\ins{s} similar functionality\ins{through bla bla bla}.
So while this might be an option, enhancing the existing tools would keep the
number of required tools low.
...
Nautilus is the default system browser in the current Pharo version\brs{specify
which version. if someone reads this 20 years from now it will probably not be
the current version}. \autoref{fig:NautilusWindow} shows how a Nautilus window
normally looks like.
...
The \textbf{selected method} is a central part of the plugin\brs{is it? It seems
to also be in the default nautilus, so how is it a central part of the plugin? I
assume you want to say that this is the main part of nautilus that TVP relies
on}.  All \chg{funtions}{functions}\brs{spell check is your friend} the plugin
...
In the \textbf{found tests droplist} every test corresponding to the selected
...
always show the test that has been selected in the found tests droplist. With
...
whenever you select a different method in the Nautilus class hierarchy.
...
\autoref{subsec:Installation and activation} \chg{let us}{we can} add a new test
...
\item Now open the droplist showing all the tests that have been found for your
...
\item Since you previously selected ``new Test'' from the droplist the plugin is
...
\item Now you need to \chg{name}{specify} \chg{in}{to} which package this new
...
\item Now expand the droplist with the results and select any test that is
...
You can verify this by opening the found tests droplist again and checking if
...
When the TVPlugin does not find tests that you have created then this might be
because it does not recognize the your test class as a possible source for tests
for the class of the method that you currently have selected in the Nautilus
class hierarchy. Here you find a step by step list of how to add your test class
to the considered test classes.

\begin{enumerate}

\item First in the Nautilus class hierarchy select the method that is
missing tests in the found tests droplist.
...
as a test class for the currently selected class you can remove this class from
...
too many tests in the found tests droplist.
...
droplist. No test contained in the unlinked class should be shown there.
...
test droplist you can add those test classes manually by following
...
If the classes that contains these wrongly identified tests do not contain any
...
If you want to remove only some tests from the found tests droplist then
...
these adjustments \ugh{although likely not for all projects and not without
changing your tests}\brs{does not parse.}. The way the TVPlugin identifies
...
sensitive. \ins{The} ":" \ins{character} from methods names with parameters will
...
selected method. This time the ":" are not removed from the check. If the
supposed test method to a method named "do:on:" does call "doOn" then it will
